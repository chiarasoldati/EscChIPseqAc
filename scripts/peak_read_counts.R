...
res_ctrl <- read.csv(file = "ctrl_peaks_for_deseq.csv")
res_rest <- read.csv(file = "restko_peaks_for_deseq.csv")

res_ctrl <- res_ctrl[,2:4]
res_rest <- res_rest[,2:4]

res_ctrl[,1] <- paste("chr",res_ctrl[,1],sep = "")
res_rest[,1] <- paste("chr",res_rest[,1],sep = "")

##remove 1 from start as bed starts at 0

res_ctrl[,"start"] <- res_ctrl[,"start"] - 1
res_rest[,"start"] <- res_rest[,"start"] - 1

write.table(res_ctrl, file = "ctrl_peaks_for_deseq.bed",row.names = F, col.names = F, quote = F)
write.table(res_rest, file = "rest_peaks_for_deseq.bed",row.names = F, col.names = F, quote = F)
...
args<-c("/mnt/data/",4, "/mnt/data/CME141_GA3R71_export_sorted_nodups.bam" ,"/mnt/data/ctrl_peaks_for_deseq.bed", "/mnt/data/CME143_GA3R71_export_sorted_nodups.bam","/mnt/data/rest_peaks_for_deseq.bed")
